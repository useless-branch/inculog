...
template<typename Serial> requires requires(
        Serial s,
        std::byte *p,
        std::size_t ss,
    ...
    { s.send(p, ss) } -> std::same_as<void>;
    { s.can_recv(dur) } -> std::same_as<bool>;
}
struct RS232 {
    std::string _dev;
            ...
            try {
                ...
                if (buffer) {
                    switch (buffer->command()) {
                        ...
                        default:
                            fmt::print("Not a known package ID: {}!\n", buffer->command());
                    }
                }
    ...
    explicit RS232(std::string const &dev) : _dev{dev} {};
