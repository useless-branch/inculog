...
namespace ts { namespace detail {
    ...
    Socket_Impl::operator FileDescriptor::View() noexcept { return get_FDView(); }
    Socket_Impl::operator const FileDescriptor::View() const noexcept { return get_FDView(); }
    ...
    FileDescriptor::View Socket_Impl::get_FDView() noexcept { return file_descriptor_.get_view(); }
    FileDescriptor::View const Socket_Impl::get_FDView() const noexcept {
        return file_descriptor_.get_view();
    }
    ...
    std::size_t Socket_Impl::send_(std::uint8_t const* buffer, std::size_t size) {
        return sendto_int_(nullptr, 0, buffer, size, 0, true);
    }
    std::size_t Socket_Impl::send_nonblocking_(std::uint8_t const* buffer, std::size_t size) {
        return sendto_int_(nullptr, 0, buffer, size, MSG_DONTWAIT, false);
    }

    std::size_t
    Socket_Impl::sendto_(Addr_t const& dst, std::uint8_t const* buffer, std::size_t size) {
        return sendto_int_(dst.get_sockaddr_ptr(), dst.size(), buffer, size, 0, false);
    }
    std::size_t Socket_Impl::sendto_nonblocking_(
      Addr_t const&       dst,
      std::uint8_t const* buffer,
      std::size_t         size) {
        return sendto_int_(dst.get_sockaddr_ptr(), dst.size(), buffer, size, MSG_DONTWAIT, false);
    }

    std::size_t
    Socket_Impl::recv_int_(std::uint8_t* buffer, std::size_t size, int flags, bool useTimeout) {
    ...
    std::size_t Socket_Impl::recv_nonblocking_(std::uint8_t* buffer, std::size_t size) {
        return recv_int_(buffer, size, MSG_DONTWAIT, !Socket::is_packet_based_type(socketType_));
    }

    std::size_t Socket_Impl::recv_some_(std::uint8_t* buffer, std::size_t size) {
        return recv_int_(buffer, size, 0, false);
    }

    std::size_t Socket_Impl::peek_nonblocking_(std::uint8_t* buffer, std::size_t size) {
        return recv_int_(
          buffer,
          size,
    ...
    std::size_t Socket_Impl::peek_some_(std::uint8_t* buffer, std::size_t size) {
        return recv_int_(buffer, size, MSG_PEEK, false);
    }

    std::size_t Socket_Impl::recv_(std::uint8_t* buffer, std::size_t size) {
        return recv_int_(buffer, size, MSG_WAITALL, !Socket::is_packet_based_type(socketType_));
    }

    std::size_t Socket_Impl::peek_(std::uint8_t* buffer, std::size_t size) {
        return recv_int_(
          buffer,
          size,
    ...
    std::size_t
    Socket_Impl::recvfrom_src_int_(Addr_t* src, std::uint8_t* buffer, std::size_t size, int flags) {
        if(src == nullptr) {
            return recv_int_(buffer, size, flags, false);
        }
    ...
    std::size_t Socket_Impl::recvfrom_src_dst_int_(
      ...
      int           flags) {
        ...
        if(dst == nullptr) {
            return recvfrom_src_int_(src, buffer, size, flags);
        }
    ...
    std::size_t Socket_Impl::recvfrom_src_dst_ifa_int_(
      ...
      int           flags) {
        ...
        if(ifa == nullptr) {
            return recvfrom_src_dst_int_(src, dst, buffer, size, flags);
        }
        if(dst == nullptr) {
    ...
    std::size_t Socket_Impl::recvfrom_(Addr_t* src, std::uint8_t* buffer, std::size_t size) {
        return recvfrom_src_int_(src, buffer, size, MSG_WAITALL);
    }

    std::size_t
    Socket_Impl::recvfrom_(Addr_t* src, Addr_t* dst, std::uint8_t* buffer, std::size_t size) {
        return recvfrom_src_dst_int_(src, dst, buffer, size, MSG_WAITALL);
    }
    std::size_t Socket_Impl::recvfrom_(
      Addr_t*       src,
      Addr_t*       dst,
      Addr_t*       ifa,
      std::uint8_t* buffer,
      std::size_t   size) {
        return recvfrom_src_dst_ifa_int_(src, dst, ifa, buffer, size, MSG_WAITALL);
    }

    std::size_t
    Socket_Impl::recvfrom_nonblocking_(Addr_t* src, std::uint8_t* buffer, std::size_t size) {
        return recvfrom_src_int_(src, buffer, size, MSG_DONTWAIT);
    }

    std::size_t Socket_Impl::peekfrom_(Addr_t* src, std::uint8_t* buffer, std::size_t size) {
        return recvfrom_src_int_(src, buffer, size, MSG_PEEK | MSG_WAITALL);
    }
    std::size_t
    Socket_Impl::peekfrom_(Addr_t* src, Addr_t* dst, std::uint8_t* buffer, std::size_t size) {
        return recvfrom_src_dst_int_(src, dst, buffer, size, MSG_PEEK | MSG_WAITALL);
    }
    std::size_t Socket_Impl::peekfrom_(
      Addr_t*       src,
      Addr_t*       dst,
      Addr_t*       ifa,
      std::uint8_t* buffer,
      std::size_t   size) {
        return recvfrom_src_dst_ifa_int_(src, dst, ifa, buffer, size, MSG_PEEK | MSG_WAITALL);
    }

    std::size_t
    Socket_Impl::peekfrom_nonblocking_(Addr_t* src, std::uint8_t* buffer, std::size_t size) {
        return recvfrom_src_int_(src, buffer, size, MSG_PEEK | MSG_DONTWAIT);
    }
        ...
        for(ifaddrs const* it = ifas; it != nullptr; it = it->ifa_next) {
            if(it->ifa_addr != nullptr) {
                if(it->ifa_addr->sa_family == AF_INET6 || it->ifa_addr->sa_family == AF_INET) {
                    ret.push_back(Addr_t(*it->ifa_addr, getLength(it->ifa_addr->sa_family)));
                }
            }
        }
#endif
    ...
    std::size_t Socket_Impl::recvfrom_int_(
      ...
      bool              useTimeout) {
        ...
        auto make_address = [](sockaddr_storage* addr, socklen_t* lenght, bool maybe_valid) {
            if(addr && maybe_valid) {
                if(*lenght != 0) {
                    return std::optional<Addr_t>{Addr_t{*addr, *lenght}};
                }
            }
        ...
        while(true) {
            ssize_t const status = [&]() {
                if(dst == nullptr) {
                    ...
                    return ::recvfrom(
                      file_descriptor_.fd(),
                      detail::next(buffer, bytesReceived),
                      size - bytesReceived,
                      flags,
                    ...
                    std::array<
                      ...
                      sizeof(in_pktinfo)
                        ...
                        + sizeof(cmsghdr)>
                          info_buffer;
                    iovec vec;
                    vec.iov_base = detail::next(buffer, bytesReceived);
