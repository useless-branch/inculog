...
namespace ts { namespace detail {
    ...
    protected:
        ...
        FileDescriptor::View       get_FDView() noexcept;
        FileDescriptor::View const get_FDView() const noexcept;
        FileDescriptor             release_FD() noexcept;

        TS_COM_MAKE_ITERATOR_TEMPLATE_CONST(send)
        TS_COM_MAKE_ITERATOR_TEMPLATE_CONST(send_nonblocking)
        TS_COM_MAKE_ITERATOR_TEMPLATE_NONE_CONST(recv)
        TS_COM_MAKE_ITERATOR_TEMPLATE_NONE_CONST(recv_nonblocking)
        TS_COM_MAKE_ITERATOR_TEMPLATE_NONE_CONST(recv_some)
        TS_COM_MAKE_ITERATOR_TEMPLATE_NONE_CONST(peek)
        TS_COM_MAKE_ITERATOR_TEMPLATE_NONE_CONST(peek_nonblocking)
        TS_COM_MAKE_ITERATOR_TEMPLATE_NONE_CONST(peek_some)

        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_REFERENCE_CONST(Addr_t, sendto)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_REFERENCE_CONST(Addr_t, sendto_nonblocking)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_POINTER_NONE_CONST(Addr_t, recvfrom)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_TYPE_POINTER_NONE_CONST(Addr_t, Addr_t, recvfrom)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_TYPE_TYPE_POINTER_NONE_CONST(
          Addr_t,
          Addr_t,
          Addr_t,
          recvfrom)

        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_POINTER_NONE_CONST(Addr_t, recvfrom_nonblocking)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_POINTER_NONE_CONST(Addr_t, peekfrom)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_TYPE_POINTER_NONE_CONST(Addr_t, Addr_t, peekfrom)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_TYPE_TYPE_POINTER_NONE_CONST(
          Addr_t,
          Addr_t,
          Addr_t,
          peekfrom)
        TS_COM_MAKE_ITERATOR_TEMPLATE_TYPE_POINTER_NONE_CONST(Addr_t, peekfrom_nonblocking)
    ...
    private:
        ...
        std::size_t send_(std::uint8_t const* buffer, std::size_t size);
        std::size_t send_nonblocking_(std::uint8_t const* buffer, std::size_t size);
        std::size_t recv_(std::uint8_t* buffer, std::size_t size);
        std::size_t recv_nonblocking_(std::uint8_t* buffer, std::size_t size);
        std::size_t recv_some_(std::uint8_t* buffer, std::size_t size);
        std::size_t peek_(std::uint8_t* buffer, std::size_t size);
        std::size_t peek_nonblocking_(std::uint8_t* buffer, std::size_t size);
        std::size_t peek_some_(std::uint8_t* buffer, std::size_t size);

        std::size_t sendto_(Addr_t const& dst, std::uint8_t const* buffer, std::size_t size);
        std::size_t
        sendto_nonblocking_(Addr_t const& dst, std::uint8_t const* buffer, std::size_t size);

        std::size_t recvfrom_(Addr_t* src, std::uint8_t* buffer, std::size_t size);
        std::size_t recvfrom_(Addr_t* src, Addr_t* dst, std::uint8_t* buffer, std::size_t size);
        std::size_t
        recvfrom_(Addr_t* src, Addr_t* dst, Addr_t* ifa, std::uint8_t* buffer, std::size_t size);
        std::size_t recvfrom_nonblocking_(Addr_t* src, std::uint8_t* buffer, std::size_t size);

        std::size_t peekfrom_(Addr_t* src, std::uint8_t* buffer, std::size_t size);
        std::size_t peekfrom_(Addr_t* src, Addr_t* dst, std::uint8_t* buffer, std::size_t size);
        std::size_t
        peekfrom_(Addr_t* src, Addr_t* dst, Addr_t* ifa, std::uint8_t* buffer, std::size_t size);
        std::size_t peekfrom_nonblocking_(Addr_t* src, std::uint8_t* buffer, std::size_t size);
        ...
        std::size_t sendto_int_(
          ...
          int                 flags,
          bool                useTimeout);
        std::size_t recv_int_(std::uint8_t* buffer, std::size_t size, int flags, bool useTimeout);
        std::size_t recvfrom_int_(
          ...
          bool              useTimeout);
        std::size_t
        recvfrom_src_int_(Addr_t* src, std::uint8_t* buffer, std::size_t size, int flags);
        std::size_t recvfrom_src_dst_int_(
          Addr_t*       src,
          Addr_t*       dst,
          std::uint8_t* buffer,
          std::size_t   size,
    ...
    public:
        ...
        static constexpr const bool isBound{true};
        static constexpr const bool isServer{false};
        static constexpr const bool isPacketBased{Socket::is_packet_based_type(Type)};
    ...
    public:
        ...
        using ClientSocket_t = Bound_ClientSocket<Protocol, Type>;
        static constexpr const bool isBound{true};
        static constexpr const bool isServer{true};
        static constexpr const bool isPacketBased{Socket::is_packet_based_type(Type)};
    ...
    public:
        ...
        static constexpr const bool isBound{false};
        static constexpr const bool isServer{false};
        static constexpr const bool isPacketBased{Socket::is_packet_based_type(Type)};
    ...
    private:
        std::size_t send_(std::uint8_t const* buffer, std::size_t size) {
            return Socket_Impl::sendto(address_, buffer, size);
        }
        std::size_t send_nonblocking_(std::uint8_t const* buffer, std::size_t size) {
            return Socket_Impl::sendto_nonblocking(address_, buffer, size);
        }
    };
