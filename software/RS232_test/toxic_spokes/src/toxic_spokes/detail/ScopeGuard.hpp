...
public:
    ...
    ScopeGuard&
      operator=(ScopeGuard const& other)
      = delete;
    ScopeGuard(ScopeGuard&& other) noexcept = default;
    ScopeGuard&
      operator=(ScopeGuard&& other) noexcept = default;
    ...
    void
      setPolicy(ScopeGuardCallPolicy p) noexcept {
        policy_ = p;
    }
...
ScopeGuard<std::decay_t<F>>
  make_scope_guard(F&& f, ScopeGuardCallPolicy p) {
