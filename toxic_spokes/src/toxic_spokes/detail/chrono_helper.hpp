...
namespace ts { namespace chrono {
    ...
    auto
      saturating_duration_cast(std::chrono::duration<Rep, Period> d) ->
    ...
    auto
      saturating_duration_cast(Duration d) {
        return d;
    }

    template<typename Rep, typename Period>
    auto
      clamp_to_positive(std::chrono::duration<Rep, Period> duration) {
    ...
    template<typename Rep, typename Period>
    auto
      to_timeval(std::chrono::duration<Rep, Period> duration) {
        using D                        = std::chrono::duration<Rep, Period>;
    ...
    auto
      calc_stop_time(std::chrono::duration<Rep, Period> timeout) {
